...
gwas_results = read.table("./PAHmerge2_MPAPdiff_white.assoc.linear", header=TRUE)
...
# Load data
dat <- read.table("./PAHmerge2_MPAPdiff_white.assoc.linear", header=TRUE)
...
g <- ggplot(don, aes(x=BPcum, y=-log10(P))) +
  ...
  geom_point(aes(color=BETA), alpha=0.8, size=1.3) +
  scale_color_gradientn(
    colours = c("navyblue","#1e90ff", "ghostwhite", "red", "darkred"),
    values = scales::rescale(c(-8,-6,-4,-2,0,2,4, 6, 8)),
    #values = c(0, 1, 2, 5),
    breaks = seq(-8,8,2),
    limits = c(-10,10)
  ) +
...
png('manhattan_3PCs.png', width=10, height=6, units ='in', res=300)
g
dev.off()
